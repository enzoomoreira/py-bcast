...
for i, line in enumerate(lines[1:], 1):
    ...
    full_sym = (
        fields[tag_305_idx]
        if tag_305_idx is not None and tag_305_idx < len(fields)
        else ""
    )
    short_ticker = (
        fields[tag_10068_idx]
        if tag_10068_idx is not None and tag_10068_idx < len(fields)
        else ""
    )
    isin = (
        fields[tag_303_idx]
        if tag_303_idx is not None and tag_303_idx < len(fields)
        else ""
    )
    name = (
        fields[tag_10045_idx]
        if tag_10045_idx is not None and tag_10045_idx < len(fields)
        else ""
    )
    exchange_id = (
        fields[tag_10092_idx]
        if tag_10092_idx is not None and tag_10092_idx < len(fields)
        else ""
    )
...
for term in search_terms:
    ...
    if not matches:
        ...
        matches = [
            ...
            if term.lower() in i["ticker"].lower()
            or term.lower() in i["full_sym"].lower()
        ]
    ...
    if matches:
        ...
        for m in matches[:5]:
            print(
                f"    {m['full_sym']:35s} ticker={m['ticker']:15s} name={m['name'][:60]}"
            )
    else:
        # Search in name
