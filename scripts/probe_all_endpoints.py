...
ENDPOINTS = [
    ...
    {
        "id": 167,
        "name": "FundamentalIndicadorHistorico",
...
def main():
    ...
    print(
        f"{'#':<4} {'ID':<4} {'ENDPOINT':<38} {'STATUS':<6} {'FMT':<12} {'SIZE':<8} {'NOTES'}"
    )
    print("-" * 120)
    ...
    for i, ep in enumerate(ENDPOINTS, 1):
        ...
        print(
            f"{i:<4} {ep['id']:<4} {name:<38} {status_str:<6} {info['format']:<12} {size_str:<8} {notes}"
        )
    ...
    working = [
        r for r in results if r["status"] == 200 and r["size"] > 100 and not r["error"]
    ]
    errors = [r for r in results if r["status"] != 200 or r.get("error")]
    empty = [
        r
        for r in results
        if r["status"] == 200 and r["size"] <= 100 and not r.get("error")
    ]
    ...
    for r in errors:
        print(
            f"   [{r['id']:>3}] {r['name']:<38} HTTP {r['status']:<5} {r['error'][:50]}"
        )
