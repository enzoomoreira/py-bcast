...
if __name__ == "__main__":
    BCAA_SESSION = get_session_token()
    client = ContentProxyClient(BCAA_SESSION)

    print("=== Closing prices 2026-05-19 ===")
    try:
        data = client.get_fechamentos(
            ["PETR4", "VALE3", "ITUB4", "BBDC4", "ABEV3"], "20260519"
        )
        for sym, vals in data.items():
            print(
                f"  {sym}: LAST={vals['last']}, SETTLE={vals['settle']}, DATE={vals['date']}"
            )
