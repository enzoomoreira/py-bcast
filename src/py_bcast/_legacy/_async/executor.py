...
from ..http import get_async_http_client, get_session_token
from ..multi import vectorize_async
...
async def _resolve_cvm_or_int(value: Any, session_token: str | None) -> int:
    """Ticker-or-CVM identifier -> CVM code (digit passthrough else resolve)."""
    if isinstance(value, int) or str(value).isdigit():
        return int(value)
    return await resolve_cvm(str(value), session_token)


async def _coerce_bind(resolve: Resolve, raw: Any, session_token: str | None) -> str:
    """Transform a bound value per its resolve policy (see :class:`ParamBind`)."""
    if resolve == "none":
        return str(raw)
    if resolve == "cvm":
        return str(await _resolve_cvm_or_int(raw, session_token))
    if resolve == "indicator":
        return str(await resolve_indicator(raw, session_token))
    if resolve == "date":
...
async def _build_params(
    ...
    for bind in spec.params:
        ...
        if raw is None:
            continue
        params[bind.tag] = await _coerce_bind(bind.resolve, raw, session_token)
    return params


@http_retry
async def _binary_fetch(
...
async def _binary_rows(
    ...
    r = await _binary_fetch(s, path, params, spec.timeout)
    try:
...
async def _fetch_rows(
    ...
    if spec.transport == "binary":
        return await _binary_rows(spec, params, inputs, session_token)
...
async def _run_single(
    ...
    params = await _build_params(spec, inputs, session_token)
    rows = await _fetch_rows(spec, params, inputs, session_token)
...
async def run_spec(
    ...
    if spec.vectorize_over is None:
        return await _run_single(spec, inputs, session_token)
